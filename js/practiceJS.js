...
function buildInScore(){
    ...
    let squaresNeeded = n + Number(numPlayers);
    for(let i = 0; i <= squaresNeeded; i++){
        $("#inScore").append(`<div id='in${i}'>i</div>`)
    }
}
...
function addScore(score, id){
    ...
    let playerString = id;
    //let sum;
    // console.log("string Id is " + playerString + " Score is "+ holeScore);
    ...
    for (let i = 1; i <= numHoles; i++){
        ...
        //console.log(" the sum is " + sum + " the value is " + value + " "  + typeof value);
        let idVar = pId + 2;
        //console.log(" pid is " + idVar);
